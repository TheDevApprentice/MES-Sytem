<template>
  <Suspense>
    <template #default>
      <div>
        <div v-if="!spreadsheetOpen" class="page-container">
          <!-- Header -->
          <div class="page-header appear appear-delay-1">
            <div class="flex flex-row items-center justify-between w-full">
              <div class="page-title">{{ title }}</div>
              <div class="page-header-button appear appear-delay-3">
                <button @click="emit('action')" class="page-btn mt-1">{{ buttonText }}</button>
              </div>
            </div>
            <!-- Stats slot -->
        ...
        <div v-if="spreadsheetOpen" class="page-container">
          <div class="page-header appear appear-delay-1">
            <div class="flex flex-row items-center justify-between w-full">
              <div class="page-title">{{ title }}</div>
              <div class="page-header-button appear appear-delay-3">
                <button @click="emit('close')" class="page-btn mt-1">{{ closeText }}</button>
              </div>
            </div>
          </div>
          <Spreadsheet :data="workbookData" :load-charts="false" />
        </div>
      </div>
    </template>
    <template #fallback>
...
import Spreadsheet from '@renderer/components/univers/Spreadsheet.vue'
...
const emit = defineEmits(['action', 'close'] as const)
</script>
