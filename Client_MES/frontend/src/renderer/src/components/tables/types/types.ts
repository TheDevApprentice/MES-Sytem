export interface TableHeader {
  text: string;
  value: string;
  sortable?: boolean;
  align?: "left" | "center" | "right";
  width?: string | number;
  type?: "text" | "image" | "status" | "currency" | "date" | "percentage" | "number" | "boolean" | "time";
  imageField?: string; // Champ optionnel pour l'URL de l'image
  formatter?: (value: any) => any;
  color?: boolean;
  format?: string;
  activeDisplay?: 'tag' | 'icon';
}

export interface TableItem {
  [key: string]: any;
}

export interface TableAction {
  name: "edit" | "delete" | "view" | "download" | string;
  icon?: string;
  color?: string;
  handler?: (item: TableItem) => void;
}

export interface SortState {
  key: string;
  order: "asc" | "desc";
}

export const TableActions: TableAction[] = [
  {
    name: "edit",
    icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>',
    color: "blue",
  },
  {
    name: "delete",
    icon: '<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>',
    color: "red",
  },
  {
    name: "view",
    icon: '<svg class="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>',
    color: "purple",
  },
  {
    name: "download",
    icon: '<svg class="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>',
    color: "dark",
  },
];

export interface LoadMorePayload {
  startIndex: number;
  endIndex: number;
}

export interface PaginationOptions {
  page: number;
  itemsPerPage: number;
  totalItems: number;
}

export interface SearchOptions {
  query: string;
  fields?: string[];
  caseSensitive?: boolean;
}

export interface TableFunctions {
  loadMore?: (payload: LoadMorePayload) => Promise<TableItem[]> | TableItem[];
  search?: (options: SearchOptions) => Promise<TableItem[]> | TableItem[];
  sort?: (sortState: SortState) => Promise<TableItem[]> | TableItem[];
}

export interface TableProps {
  headers: TableHeader[];
  items: TableItem[];
  loading?: boolean;
  selectable?: boolean;
  showActions: boolean;
  selectedItems?: TableItem[];
  pagination?: PaginationOptions;
  sortBy?: SortState;
  functions?: TableFunctions;
  actions: TableAction[];
}

export interface TableEmits {
  (e: "update:selectedItems", items: TableItem[]): void;
  (e: "update:sortBy", sort: SortState): void;
  (e: "update:pagination", pagination: PaginationOptions): void;
  (e: "rowClick", item: TableItem): void;
}

export * from "./types";
